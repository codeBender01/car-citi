...
import type { NewOfferType } from "@/interfaces/carSpecs.interface";

interface AddOfferTypeModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  formData: NewOfferType;
  setFormData: Dispatch<SetStateAction<NewOfferType>>;
  onSubmit: (e: React.FormEvent) => void;
}

export const AddOfferTypeModal = ({
  open,
  onOpenChange,
  formData,
  setFormData,
  onSubmit,
}: AddOfferTypeModalProps) => {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[500px]">
        <DialogHeader>
          <DialogTitle>
            {formData.id
              ? "Редактировать тип предложения"
              : "Добавить тип предложения"}
          </DialogTitle>
          <DialogDescription>
            {formData.id
              ? "Обновите данные типа предложения"
              : "Создайте новый тип предложения"}
          </DialogDescription>
        </DialogHeader>
