...
interface AddNewsCategoryModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  formData: NewNewsCategory;
  setFormData: Dispatch<SetStateAction<NewNewsCategory>>;
  onSubmit: (e: React.FormEvent) => void;
}

export const AddNewsCategoryModal = ({
  open,
  onOpenChange,
  formData,
  setFormData,
  onSubmit,
}: AddNewsCategoryModalProps) => {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[500px]">
        <DialogHeader>
          <DialogTitle>Добавить категорию новостей</DialogTitle>
          <DialogDescription>
            Создайте новую категорию для новостей
          </DialogDescription>
        </DialogHeader>
